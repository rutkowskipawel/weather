                                ...
                                public void onNext(WeatherResponse weatherResponse) {
                                    try (Realm realm = Realm.getDefaultInstance()) {
                                        realm.executeTransaction(realm1 -> {
                                            Weather newWeather = realm.createObject(Weather.class);
                                            DataMapper.transformWeatherResponseToWeather(weatherResponse, newWeather);
                                            weather.set(realm.copyFromRealm(newWeather));
                                        });
                                    }
                                }
                            }));
